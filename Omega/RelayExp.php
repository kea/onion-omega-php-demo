...
class RelayExp
{
    ...
    public function __construct($switch = '000', $initialize = true)
    {
        ...
        $this->switch = $switch;
        $this->address = self::switchToAddress($switch);
    ...
    static public function addressToSwitch($address)
    {
        $address = sprintf("%03s", decbin(7 - $address));

        return $address[2].$address[1].$address[0];
    }

    static public function switchToAddress($switch)
    {
        return 7 - bindec($switch[2].$switch[1].$switch[0]);
    }

    static public function scan($from = 0, $to = 7)
    {
        $actives = [];
        $bInitialized = null;
        for ($i = $from; $i <= $to; $i++) {
            $active = \relayCheckInit($i, $bInitialized);
            if ($active) {
                $actives[] = $i;
            }
        }
